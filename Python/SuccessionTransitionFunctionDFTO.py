...
def ed_trans_prob(*args):
    ...
    plt.title("Transition Probability for DFTO, Early Development")
...
def md_trans_prob(*args):
    ...
    for label, PS, SY, SL in args:
        ...
        transition_prob[AS >= SL] = 0
        plt.plot(AS, transition_prob, label=label)
    plt.title("Transition Probability for DFTO, Mid Development")
    plt.xlabel("Years in Stage")
    plt.ylabel("Probability of patch remaining in Mid Development")
    plt.legend()


def ld_trans_prob(*args):
    ...
    for label, PS, SY, SL in args:
        ...
        transition_prob[AS >= SL] = 0
        plt.plot(AS, transition_prob, label=label)
    plt.title("Transition Probability for DFTO, Late Development")
    plt.xlabel("Years in Stage")
