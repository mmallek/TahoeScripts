...
def ed_trans_prob(*args):
    ...
    plt.title("Transition Probability for OCFW, Early Development")
...
def mdc_trans_prob(*args):
    ...
    for label, PS, SY, SL in args:
        ...
        transition_prob[AS >= SL] = 0
        plt.plot(AS, transition_prob, label=label)
    plt.title("Transition Probability for OCFW, Mid Development Closed")
    plt.xlabel("Years in Stage")
...
def mdo_trans_prob(*args):
    ...
    for label, PS, SY, SL in args:
        ...
        transition_prob[AS >= SL] = 0
        plt.plot(AS, transition_prob, label=label)
    plt.title("Transition Probability for OCFW, Mid Development Open")
    plt.xlabel("Years in Stage")
...
def lda_trans_prob(*args):
    ...
    for label, PS, SY, SL in args:
        ...
        transition_prob[AS >= SL] = 0
        plt.plot(AS, transition_prob, label=label)
    plt.title("Transition Probability for OCFW, Late Development")
    plt.xlabel("Years in Stage")
