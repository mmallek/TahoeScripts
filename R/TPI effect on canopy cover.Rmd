...
ccavg.r3res.samp = read.csv('/Users/mmallek/Tahoe/GISdata/HRV_201506/ccavg_r3res_smooth_clip_20150701_samp.csv')
...
# this won't run because I don't have "loopdata" - not sure what it is (7/2015)
for (i in PlotLevels){
    plot(x = loopdata$tpi, 
     y = loopdata$ccavg,
     xlab = "TPI",
...
covertypes = c('LPN','LPN_ASP','MEG_M','MEG_U','MEG_X','MRIP',
               'OCFW','OCFW_U','RFR_M','RFR_U','RFR_X','SCN','SMC_ASP',
               'SMC_M','SMC_U','SMC_X',"WWP")
# covcodes of interest: 3,5,6,9,10,11,12,14,15,17,18,19,21,23,24,25,26,29
covers = as.data.frame(covertypes)
dict = as.data.frame(sort(unique(data$cover)))
names(dict) = "covcode"
dict = dict[c(4,5,7:10,12,13,15:17,18:22,25),]
dict = as.data.frame(dict)

dict2 = cbind(dict, covers)
require(plyr)
dict2 = rename(dict2, c('dict'='cover', 'covertypes'='covname'))
...
```{r}
# does this chunk have a purpose?
...
subtable
write.table(subtable, file="/Users/mmallek/Tahoe/RMLands/results/hrv/cctable.txt", sep='\t')
write.csv(subtable, file="/Users/mmallek/Tahoe/RMLands/results/hrv/cctable.csv")
