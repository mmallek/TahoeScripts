...
```{r}
theme_set(theme_bw())
darea = read.csv('/Users/mmallek/Tahoe/RMLands/results/hrv/Wildfire_darea_trajectory.csv', header=TRUE)
...
p3 = ggplot() + theme_gray() + geom_line(data=climdarea, aes(timestep, climate)) + 
    ...
    theme(axis.title.x = element_text(size=30, vjust=0), 
...
ndarea = read.csv(paste(hrv000path,'darea.csv',sep=''))
ndarea = subset(ndarea, session.id==2 & timestep.id >= 40)     
...
pl + geom_bar(stat="identity") + theme_bw() + ylab("Proportion of Fires") + 
    xlab("Fire Size") +
    theme(axis.title.x = element_text(size=30, vjust=-2), 
          axis.title.y = element_text(size=30, vjust=2),
