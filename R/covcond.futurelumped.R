# this script makes the covcond plots but lumps all the future scenarios together
...
imagepath = "/Users/mmallek/Tahoe/RMLands/results201507/future/covcondboxplots_bycover_lumped/"
hrvsession = 3
...
covcond.cover.boxplot <-
    function(path,sessions=NULL,var='srv50%',runs=NULL,start.step=1,
        ...
        y00<-y00[y00$cov.name %in% t2.cover.type,]
          ...
          # this gets you the data frame with only the timesteps that you want
            ...
            for(i in 1:length(unique(s2$cov.name))){
                s2[s2$cov.name==unique(s2$cov.name)[i],4] = s2[s2$cov.name==unique(s2$cov.name)[i],3] *.09/t2[i,3]
            }
            current = merge(y1, s2)
        ...
        for(i in 1:nrow(t2)){ # unique cover types
            y[y$cov.name == t2$cover.type[i],8] = # matches cover type, goes in proportion column
                y[y$cov.name == t2$cover.type[i],7] *.09/t2$area.ha[i]
        }
        ...
        #loop thru selected sessions
            for(j in 1:length(sessions)){
                ...
                data_long = gather(z1, run_ts, proportion, -cov.cond.id, -cov.name, -cond.name)
                #data_long$ = relevel(z$scenario, "pastclimate")
                
                
                for(i in 1:length(unique(y$cov.name))){
                    p = ggplot(data=y[y$cov.name==unique(y$cov.name)[i],], aes(x=cond.name, y=proportion )) 
                    p1 = p +                
                        geom_crossbar(data=current[current$cov.name==unique(y$cov.name)[i],], 
